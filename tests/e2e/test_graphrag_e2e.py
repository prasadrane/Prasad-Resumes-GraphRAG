...
class TestGraphRAGE2E:
    ...
    @pytest.fixture
    def client(self):
        """Create test client pointing at FastAPI app."""
    ...
    def test_query_all_modes(self, client: TestClient):
    ...
    def test_chat_stream_endpoint(self, client: TestClient):
        ...
        assert "data:" in resp.text  # SSE format

    @pytest.mark.skipif(not _has_api_key(), reason="No LLM API key available")
    def test_conversation_memory_persistence(self, client: TestClient):
        """Test that conversation history persists across requests."""
        session = "e2e-history-test"

        # First message
        resp1 = client.post("/api/chat-stream", json={
    ...
    def test_invalid_mode_default_to_local(self, client: TestClient):
    ...
    @pytest.mark.skipif(not _has_api_key(), reason="No LLM API key available")
    def test_drift_mode_multi_hop(self, client: TestClient):
